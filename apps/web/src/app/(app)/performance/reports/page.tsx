    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden p-6">
      <div className="max-w-5xl mx-auto w-full">
        <header className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-black text-foreground mb-2">Performance Reports</h1>
            <p className="text-sm text-muted-foreground">
              View detailed insights and actionable recommendations for your past audits.
            </p>
          </div>
        </header>
        ...
        {isLoading ? (
          ...
          <div className="text-center p-12 border rounded-xl bg-card">
            ...
            <p className="text-sm text-muted-foreground">Run a performance audit to generate a report.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4">
            {reports.map((audit, index) => (
              <Link key={audit.id} href={`/performance/reports/${audit.id}`}>
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: index * 0.05 }}
                  className="border border-border rounded-xl bg-card p-4 hover:shadow-lg hover:border-primary/50 transition-all cursor-pointer group"
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className={cn("shrink-0 flex items-center justify-center w-10 h-10 rounded-lg font-black border text-lg", getGradeColor(audit.performance_score))}>
                      {getGradeLetter(audit.performance_score)}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-border/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold">{audit.performance_score ?? '-'}</p>
                    </div>
                  </div>
                </motion.div>
              </Link>
