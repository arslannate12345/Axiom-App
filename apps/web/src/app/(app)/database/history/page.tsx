...
export default function DatabaseHistoryPage() {
  const [history, setHistory] = useState<DatabaseAudit[]>([]);
    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto p-6">
      <div className="max-w-5xl mx-auto w-full space-y-6">
        <header className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-black text-foreground mb-1">Database Audit History</h1>
            <p className="text-sm text-muted-foreground">
              Review past database health checks, query performance, and schema validation runs.
            </p>
          </div>
          <Link
            href="/database"
        ...
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-card border border-border/80 rounded-2xl p-12 text-center shadow-sm">
            <span className="material-symbols-outlined text-5xl text-muted-foreground mb-3">database</span>
            <h3 className="text-lg font-bold text-foreground mb-1">No Database Audits Found</h3>
            <p className="text-sm text-muted-foreground mb-6 max-w-sm mx-auto">
              Run your first database health audit to inspect connection latency, schema integrity, and endpoint readiness.
            </p>
            <Link
              href="/database"
                ...
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                      ...
                      <span>•</span>
                      <span>{audit.summary.avgLatencyMs}ms avg latency</span>
                      <span>•</span>
                      <span>{audit.summary.passedCount} checks passed</span>
                    </p>
                  </div>
                </div>
